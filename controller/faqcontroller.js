...
const getFAQs = asyncHandler(async (req, res) => {
  try {
    ...
    if (cachedFAQs) {
      return res.status(200).send(JSON.stringify(new APIResponse(200, JSON.parse(cachedFAQs), "")));
    }
    ...
    if (lang !== "en") {
      faqs = faqs.map((faq) => {
        console.log("Working");
        const translatedFAQ = { id: faq._id, ...faq.getTranslation(lang) };
        console.log(translatedFAQ);
    ...
    // Send the response using res.send(), converting the object to a string
    return res.status(200).send(JSON.stringify(new APIResponse(200, faqs, "")));
  } catch (error) {
    console.log(error);
